...
BlueXP  Workload FactoryでストレージからFSx for ONTAPの新しいクレデンシャルを追加する場合は、どの権限レベル（_operational mode_）で操作するかを決定する必要があります。FSx for ONTAPファイルシステムなどのAWSリソースを検出して導入するには、_Read-only_or_Read/Write_permissionsが必要です。BlueXP  FSx for ONTAPは、_Read-only_modeまたは_Read/Write_modeを選択しないかぎり、_basic_modeで動作します。_読み取り専用_は、表示権限と同じです。_読み取り/書き込み_は、操作権限と同じです。link:https://docs.netapp.com/us-en/workload-setup-admin/operational-modes.html["動作モードの詳細"]です。
...
. アクセス許可レベル（[Read/Write]または[Read-Only]）を選択できるワークロード機能については、これらのクレデンシャルで使用できるアクセス許可のタイプを選択します。link:https://docs.netapp.com/us-en/workload-setup-admin/operational-modes.html["権限（運用モードとも呼ばれます）について説明します。"^]です。
